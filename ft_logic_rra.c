...
/*   Updated: 2026/04/25 17:07:44 by sihasima         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_logic_reverse_rotate(t_node **stack)
{
	t_node	*first;
	t_node	*nw_first;
	t_node	*av_last;
...
{
	ft_logic_reverse_rotate(stack_a);
	write(1, "rra\n", 4);
...
{
	ft_logic_reverse_rotate(stack_b);
	write(1, "rrb\n", 4);
	if (config && config->bench_mode == 1)
	{
		config->count_rrb++;
